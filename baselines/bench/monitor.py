...
class Monitor(Wrapper):
    ...
    def reset(self, **kwargs):
        ...
        for k in self.reset_keywords:
            v = kwargs.get(k)
            if v is None:
                raise ValueError('Expectd you to pass kwarg %s into reset' % k)
    ...
    def reset_state(self):
        if not self.allow_early_resets and not self.needs_reset:
            raise RuntimeError("Tried to reset an environment before done. " +
                               " If you want to allow early resets, wrap" +
                               " your env with" +
...
def load_results(dir):
    ...
    if not monitor_files:
        raise LoadMonitorResultsError("no monitor files of the form" +
                                      " *%s found in %s" % (Monitor.EXT, dir))
    dfs = []
    headers = []
...
def test_monitor():
    ...
    assert set(metadata.keys()) == {'env_id', 'gym_version', 't_start'},\
        "Incorrect keys in monitor metadata"

    last_logline = pandas.read_csv(f, index_col=None)
    assert set(last_logline.keys()) == {'l', 't', 'r'},\
